...
# [인터뷰 설명] From a production standpoint, we must consider the memory footprint. In a worst-case scenario where the grid is massive and infections peak, storing O(N by M) coordinates in a raw deque could cause an Out-Of-Memory (OOM) error. If I were designing this for a large-scale real-world network, I would consider a batched or sliding-window processing model to throttle memory consumption.
# [해석]        실제 운영 환경 관점에서 우리는 메모리 사용량을 고려해야 합니다. 그리드가 거대하고 오염이 정점에 달하는 최악의 시나리오에서는 생 큐(Raw deque)에 O(N*M)개의 좌표를 저장하다가 OOM(메모리 고갈) 에러가 날 수 있습니다. 만약 대규모 실무 네트워크를 위해 이걸 설계한다면, 메모리 소비를 조절하기 위해 배치 처리나 슬라이딩 윈도우 모델을 고려할 것입니다.
...
# ------------------------------------------------------------------------------
# [인터뷰 설명] Ultimately, this grid problem is a elegant, simplified simulation of dependency propagation and blast radius analysis in distributed networks.
